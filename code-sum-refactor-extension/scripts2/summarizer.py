...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Summarize source code with the bundled T5 model."
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    if args.stdin and args.base64_input is not None:
        parser.error("use either --stdin or base64_input, not both")
    if not args.stdin and args.base64_input is None:
        parser.error("either --stdin or base64_input is required")
